...
Focusing on negative news initially, the sample average abnormal returns of the event date at $(t=0)$ is significantly negative at -0.40\% at a 1\% level. Investors are reacting to spikes in bad news of firms by selling their shares. The days following an event demonstrates no abnormal performance. 

The effect is approximately zero on average until $t = -5$ upon which the AAR decreases in the days leading up to the event date, with the only significantly negative AARs found between $t=-5$ and $t=0$ at a $5\%$ level.  Nonetheless, the CAAR is significantly negative from $t=-5$ and through the remaining window where it bottoms at approximately $-1.5\%$ 10 days after an event. The new information appear to be priced in earlier than the spike in news occurs. 
