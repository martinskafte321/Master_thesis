...
list = list(1,4,8,12)
alpha_positive <- data.frame(matrix(nrow=4, ncol=9)) 
colnames(alpha_positive)<-c("Alpha","t","p" ,"return","index_return", "ave_N","holding_period","rolling_mean","SD_thres")
...
alpha_M_positive = long_term_data_M_positive %>% 
   summarise(alpha = tidy(lm(ret - RF ~ 1 + mkt_excess_ret + SMB + HML + RMW + CMA))$estimate[1],
             t = tidy(lm(ret - RF ~ 1 + mkt_excess_ret + SMB + HML + RMW + CMA))$statistic[1],
             p = tidy(lm(ret - RF ~ 1 + mkt_excess_ret + SMB + HML + RMW + CMA))$p.value[1],
...
print(xtable(as.data.frame(emptylist2[[2]][3]),type="latex",
             digits=c(0,4,2,2,2,0,1,1,1,1),
             hline.after=c(-1,0),
             floating=FALSE,latex.environments=NULL),
      include.rownames=FALSE,
      include.colnames = TRUE) %>% 
  write(file="alpha_positive_FF3_1.tex")
```
...
list = list(1,4,8,12)
alpha_negative <- data.frame(matrix(nrow=4, ncol=9)) 
colnames(alpha_negative)<-c("Alpha","t","p" ,"return","index_return", "ave_N","holding_period","rolling_mean","SD_thres")
...
print(xtable(as.data.frame(emptylist_neg_2[[5]][2]),type="latex",
             digits=c(0,4,2,3,2,0,1,1,1,1),
             hline.after=c(-1,0),
             floating=FALSE,latex.environments=NULL),
      include.rownames=FALSE,
      include.colnames = TRUE) %>% 
  write(file="alpha_negative_FF3_5.tex")
