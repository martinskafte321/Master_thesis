...
long_term_data = long_term_data_weekly_positive %>% select(date,isin,ret,contains("-")) %>%
  pivot_longer(!c(date,isin,ret), names_to = "period", values_to = "event") %>%
...
# 1 Month portfolio returns:

long_term_data_1M = long_term_data %>%
...
long_term_data_1M %>%
  ggplot(aes(x = date, y = ret)) + 
   ...
   scale_x_date(date_breaks = "1 years", date_labels = "%Y" )
      ggsave("LT_positive_returns_1.png")
  

  
# 3 Month portfolio returns:
long_term_data_3M = long_term_data %>%
  # Only include the last 1 months:
  filter(period >= -3) %>%
  group_by(date) %>%
  summarise(ret = mean(ret)) %>%
  ...
  ggplot(aes(x = date, y = ret)) + 
   ...
   labs(title = "T = 3",
    ...
    scale_y_continuous(breaks = pretty_breaks(),
                     labels = scales::percent) +
   scale_x_date(date_breaks = "1 years", date_labels = "%Y" )
      ggsave("LT_positive_returns_3.png")

# 6 Month portfolio returns:
long_term_data_6M = long_term_data %>%
  ...
  ggplot(aes(x = date, y = ret)) + 
   ...
   labs(title = "T = 6",
    ...
    scale_y_continuous(breaks = pretty_breaks(),
                     labels = scales::percent) +
   scale_x_date(date_breaks = "1 years", date_labels = "%Y" )
      ggsave("LT_positive_returns_6.png")
      
# 12 Month portfolio returns:
long_term_data_12M = long_term_data %>%
  # Only include the last 1 months:
  filter(period >= -12) %>%
  group_by(date) %>%
  summarise(ret = mean(ret)) %>%
  ...
  ggplot(aes(x = date, y = ret)) + 
   ...
   labs(title = "T = 12",
    ...
    scale_y_continuous(breaks = pretty_breaks(),
                     labels = scales::percent) +
   scale_x_date(date_breaks = "1 years", date_labels = "%Y" )
      ggsave("LT_positive_returns_12.png")
      
table_positive = rbind(alpha_1M,alpha_3M,alpha_6M,alpha_12M) %>% relocate(model)

print(xtable(table_negative,type="latex",
             digits=c(0,4,4,4,4),
             hline.after=c(-1,0),
             floating=FALSE,latex.environments=NULL),
      include.rownames=FALSE,
      include.colnames = TRUE) %>% 
  write(file="alpha_12M_negative.tex")
...
long_term_data_negative = long_term_data_weekly_negative %>% select(date,isin,ret,contains("-")) %>%
  pivot_longer(!c(date,isin,ret), names_to = "period", values_to = "event") %>%
  mutate(period = as.numeric(period)) %>%
  filter(event == '1') %>%
    # Remove duplicate rows of the returns in case one ISIN pops up on several dates. 
...
long_term_data_1M_negative = long_term_data_negative %>%
  # Only include the last 1 months:
  filter(period >= -1) %>%
  group_by(date) %>%
  summarise(ret = mean(ret)) %>%
...
alpha_1M_negative = long_term_data_1M_negative %>%  
  summarise(alpha = tidy(lm(ret ~ 1 + mkt_excess + SMB + HML))$estimate[1],
            t = tidy(lm(ret ~ 1 + mkt_excess + SMB + HML))$statistic[1],
            p = tidy(lm(ret ~ 1 + mkt_excess + SMB + HML))$p.value[1]) %>%
  mutate(model = "1M"
  ...
  ggplot(aes(x = date, y = ret)) + 
   ...
   labs(title = "T = 1",
    ...
    scale_y_continuous(breaks = pretty_breaks(),
                     labels = scales::percent) +
   scale_x_date(date_breaks = "1 years", date_labels = "%Y" )
      ggsave("LT_negative_returns_1.png")
  

  
# 3 Month portfolio returns:
long_term_data_3M_negative = long_term_data_negative %>%
  # Only include the last 1 months:
  filter(period >= -3) %>%
  group_by(date) %>%
  summarise(ret = mean(ret)) %>%
...
alpha_3M_negative = long_term_data_3M_negative %>%  
  summarise(alpha = tidy(lm(ret ~ 1 + mkt_excess + SMB + HML))$estimate[1],
            t = tidy(lm(ret ~ 1 + mkt_excess + SMB + HML))$statistic[1],
            p = tidy(lm(ret ~ 1 + mkt_excess + SMB + HML))$p.value[1]) %>%
  mutate(model = "3M"
  ...
  ggplot(aes(x = date, y = ret)) + 
   ...
   labs(title = "T = 3",
    ...
    scale_y_continuous(breaks = pretty_breaks(),
                     labels = scales::percent) +
   scale_x_date(date_breaks = "1 years", date_labels = "%Y" )
      ggsave("LT_negative_returns_3.png")

# 6 Month portfolio returns:
long_term_data_6M_negative = long_term_data_negative %>%
  # Only include the last 6 months:
  filter(period >= -6) %>%
  group_by(date) %>%
  summarise(ret = mean(ret)) %>%
...
alpha_6M_negative = long_term_data_6M_negative %>%  
  summarise(alpha = tidy(lm(ret ~ 1 + mkt_excess + SMB + HML))$estimate[1],
            t = tidy(lm(ret ~ 1 + mkt_excess + SMB + HML))$statistic[1],
            p = tidy(lm(ret ~ 1 + mkt_excess + SMB + HML))$p.value[1]) %>%
  mutate(model = "6M"
  ...
  ggplot(aes(x = date, y = ret)) + 
   ...
   labs(title = "T = 6",
    ...
    scale_y_continuous(breaks = pretty_breaks(),
                     labels = scales::percent) +
   scale_x_date(date_breaks = "1 years", date_labels = "%Y" )
      ggsave("LT_negative_returns_6.png")
      
# 12 Month portfolio returns:
long_term_data_12M_negative = long_term_data_negative %>%
  # Only include the last 1 months:
  filter(period >= -12) %>%
  group_by(date) %>%
  summarise(ret = mean(ret)) %>%
...
alpha_12M_negative = long_term_data_12M_negative %>%  
  summarise(alpha = tidy(lm(ret ~ 1 + mkt_excess + SMB + HML))$estimate[1],
            t = tidy(lm(ret ~ 1 + mkt_excess + SMB + HML))$statistic[1],
            p = tidy(lm(ret ~ 1 + mkt_excess + SMB + HML))$p.value[1]) %>%
  mutate(model = "12M"
  ...
  ggplot(aes(x = date, y = ret)) + 
   ...
   labs(title = "T = 12",
    ...
    scale_y_continuous(breaks = pretty_breaks(),
                     labels = scales::percent) +
   scale_x_date(date_breaks = "1 years", date_labels = "%Y" )
      ggsave("LT_negative_returns_12.png")
      
table_negative = rbind(alpha_1M_negative,alpha_3M_negative,alpha_6M_negative,alpha_12M_negative) %>% relocate(model)
...
remove(short_term_data_sdg,sentiment_daily,data_daily)
# Create columns that represent the lagged and leading values in relation to the event date:
