...
for (y in 1:length(looplist)) {
  
  for (z in 1:length(looplist2)) { 
...
long_term_data_monthly_positive = long_term_data_monthly 

# Add lagged values of the events to sort whether an event has happened in the last X months. 
  for (i in 1:12) {
  long_term_data_monthly_positive[paste0("-",i)] <- long_term_data_monthly_positive %>% group_by(isin) %>% transmute(t = lag(pos_event,i)) %>% ungroup() %>% select(-isin)
  }


long_term_data_positive = long_term_data_monthly_positive %>% select(date,isin,ret,MKT_CAP,free_float_mkt_cap,contains("-")) %>%
...
colnames(alpha_positive)<-c("Alpha","t","p" ,"return","index_return", "model")
...
long_term_data_M_positive = long_term_data_positive %>% 
  # Only include the last i months:
  filter(period >= -i) %>%
  group_by(date) %>%
  summarise(ret = weighted.mean(ret,free_float_mkt_cap),
            n = n_distinct(isin)) %>%
...
alpha_M_positive = long_term_data_M_positive %>% 
   summarise(alpha = tidy(lm(ret - RF ~ 1 + mkt_excess_ret + SMB + HML))$estimate[1],
             t = tidy(lm(ret - RF ~ 1 + mkt_excess_ret + SMB + HML))$statistic[1],
             p = tidy(lm(ret - RF ~ 1 + mkt_excess_ret + SMB + HML))$p.value[1],
             return = 100*mean(12*(ret-RF)),
             index_return = 100*mean(12*mkt_excess_ret)) %>%
   mutate(model = paste0(z,y,i,"M")) 

alpha_positive[x,] = alpha_M_positive

}
emptylist1[[z]] = alpha_positive
}
emptylist2[[y]] = emptylist1
}
...
long_term_data_monthly_negative = long_term_data_monthly

# Add lagged values of the events to sort whether an event has happened in the last X months. 
  for (i in 1:12) {
  long_term_data_monthly_negative[paste0("-",i)] <- long_term_data_monthly_negative %>% group_by(isin) %>% transmute(t = lag(neg_event,i)) %>% ungroup() %>% select(-isin)
  }

long_term_data_negative = long_term_data_monthly_negative %>% select(date,isin,ret,MKT_CAP,free_float_mkt_cap,contains("-")) %>%
  pivot_longer(!c(date,isin,ret,MKT_CAP,free_float_mkt_cap), names_to = "period", values_to = "event") %>%
  mutate(period = as.numeric(period)) %>% 
  filter(event == '1') %>%
    # Remove duplicate rows of the returns in case one ISIN pops up on several dates. 
...
colnames(alpha_negative)<-c("Alpha","t","p" ,"return","index_return", "model")
...
long_term_data_monthly %>% filter(date >= '2018-01-01') %>% group_by(date) %>% summarise(ret = 100*weighted.mean(ret,MKT_CAP)) %>% arrange(desc(date)) %>%left_join(eu_3factors_monthly, by = "date") %>% transmute(date, ret = ret - RF, mkt_excess = 100*mkt_excess) %>% ungroup() %>% summarise(ret = mean(ret), excess = mean(mkt_excess))
```
