...
alpha_M_negative = long_term_data_M_negative %>% 
   summarise(alpha = tidy(lm(ret - RF ~ 1 + mkt_excess_ret + SMB + HML + RMW + CMA, weights = n))$estimate[1],
            ...
            r = glance(lm(ret - RF ~ 1 + mkt_excess_ret + SMB + HML + RMW + CMA, weights = n))$r.squared,
            ret = 100*mean(12*ret),
            mkt = 100*mean(12*mkt_excess_ret),
             N = mean(n)) %>%
   mutate(model = paste0(i,"M_",y,"_",z))
...
long_term_data_M_negative %>%
  summarise(tidy(summary(lm(ret - RF ~ 1 + mkt_excess_ret + SMB + HML + RMW + CMA, weights = n))))

model = lm(ret - RF ~ 1 + mkt_excess_ret + SMB + HML + RMW + CMA, weights = n, data = long_term_data_M_negative)
...
negative %>% filter(metric == 'CAAR') %>% mutate(ESG_risk_category = factor(ESG_risk_category, levels = c("Low", "Medium","High"))) %>% 
  ...
  negative_index_all %>% filter(type == '0') %>% group_by(date) %>% summarise(ret = 100*weighted.mean(12*value, MKT_CAP))
negative_index_all %>% filter(date == '2018-05-25') %>% filter(type == '0') %>% group_by(date) %>% summarise(ret = weighted.mean(value, MKT_CAP))

negative_index_all %>% filter(type == '0') %>% group_by(date) %>% summarise(ret = weighted.mean(value, MKT_CAP))
```
