...
alpha_positive <- data.frame(matrix(nrow=4, ncol=10)) 
...
short_term_data_daily = data_daily %>% 
 ...
 ##### BP-test of the individual stock data:
 
data = short_term_data_daily %>% select(date,isin,W_RETURN,mkt_excess) %>% na.omit()
   
 test = data %>% nest(data = -isin) %>% mutate(tidied = map(data, ~ bptest(W_RETURN ~ mkt_excess, data = .)))
