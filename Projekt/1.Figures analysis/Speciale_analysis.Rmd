...
# Plots
  ...
  positive_index %>% select(type,AAR,CAAR) %>%
    pivot_longer(!c(type), names_to = "metric", values_to = "ret") %>%
   left_join(
     positive_index %>% select(type, "AAR" = SE_AAR, "CAAR" = SE_CAAR) %>% pivot_longer(!c(type), names_to = "metric", values_to = "SE"), by = c("metric","type")
   ) %>%
   mutate(
     low = ret - 1.96*SE,
     high = ret + 1.96*SE
...
negative_index %>% select(type,AAR,CAAR) %>%
    pivot_longer(!c(type), names_to = "metric", values_to = "ret") %>%
   left_join(
     negative_index %>% select(type, "AAR" = SE_AAR, "CAAR" = SE_CAAR) %>% pivot_longer(!c(type), names_to = "metric", values_to = "SE"), by = c("metric","type")
   ) %>%
   mutate(
     low = ret - 1.96*SE,
     high = ret + 1.96*SE
...
test = short_term_data_event %>%
  ...
  filter(type == '+10') %>% ungroup() %>% select(csum)
   # group_by(type) %>%
   # summarise(sd = sd(csum, na.rm = TRUE), n = n(), se = sd / sqrt(n))
...
test_neg = short_term_data_event %>%
  filter(neg_event == 1) %>%
  select(-c(pos_event,neg_event,positive_threshold,negative_threshold,norm_positive_sum,norm_negative_sum)) %>%
  pivot_longer(!c(date,isin), names_to = "type", values_to = "value") %>%
  na.omit(value) %>% 
