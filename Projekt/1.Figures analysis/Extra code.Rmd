...
all %>%
  ...
  ggplot(
    aes(x = sdg, y = AAR, color= sdg,group = sdg, fill = sdg)) + 
  geom_bar(stat="identity") +
  geom_errorbar( aes(x=sdg, ymin=AAR-CI, ymax=AAR+CI), colour = "orange", width=0.5, alpha=0.8, size=0.7) + 
  scale_y_continuous(name ="Abnormal return",
                     labels = scales::percent) +
  labs(x = "SDG group") +
  theme(legend.title = element_blank(),
...
significance_pos_2 = positive_index_2 %>% 
 ...
 se_pos_2 = significance_pos_2 %>%
  group_by(type) %>%
  summarise(sd = sd(csum, na.rm = TRUE),
            ret = weighted.mean(csum,MKT_CAP))
