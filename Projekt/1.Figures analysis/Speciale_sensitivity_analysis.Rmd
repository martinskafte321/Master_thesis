...
negative_index_10_e = negative_index_all %>% 
  mutate(csum = cumsum(value)) %>%
  group_by(type) %>%
  summarise(
    AAR = mean(value),
...
positive_index_10_sensi %>% select(type, "AAR_sens" = AAR, "CAAR_sens" = CAAR) %>%
  left_join(negative_index_10_2sd %>% select(type,AAR,CAAR), by ="type") %>%
...
negative_index_10_v %>% select(type, "AAR_v" = AAR, "CAAR_v" = CAAR) %>%
  left_join(negative_index_10_e %>% select(type,AAR,CAAR), by ="type") %>%
  pivot_longer(!type, names_to = "metric", values_to = "ret") %>%
  mutate(
    weight = 
           case_when(
...
negative_index_10_1sd %>% select(type,"AAR1sd" = AAR, "CAAR1sd" = CAAR) %>%
  left_join(negative_index_10_2sd %>% select(type,"CAAR2sd" = CAAR, "AAR2sd" = AAR), by ="type") %>%
   left_join(negative_index_10_3sd %>% select(type,"CAAR3sd" = CAAR, "AAR3sd" = AAR), by ="type") %>%
  pivot_longer(!type, names_to = "metric", values_to = "ret") %>%
  mutate(
    group = 
           case_when(
