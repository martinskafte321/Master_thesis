...
negative_index_e = negative_index_all %>% 
  mutate(csum = cumsum(value)) %>%
  group_by(type) %>%
  summarise(
    AAR = mean(value),
...
negative_index_10_1sd %>% select(type,"AAR_1sd" = AAR, "CAAR_1sd" = CAAR) %>%
  left_join(negative_index_10_2sd %>% select(type,"CAAR_2sd" = CAAR, "AAR_2sd" = AAR), by ="type") %>%
   left_join(negative_index_10_3sd %>% select(type,"CAAR_3sd" = CAAR, "AAR_3sd" = AAR), by ="type") %>%
  pivot_longer(!type, names_to = "metric", values_to = "ret") %>%
  mutate(
    group = 
           case_when(
