...
index_daily <- read.csv("C:/Users/Marti/OneDrive - University of Copenhagen/KU/Speciale/Data behandling/MSCI_World_daily.csv", 
                             sep = ",") %>%
...
testdata = short_term_data %>% filter(isin == 'US92343V1044') %>% ungroup() %>% select(date,abnormal_ret) %>% mutate(date = as.POSIXct(date))
...
short_term_data <- short_term_data_daily %>%
  group_by(isin) %>%
  mutate(
      pred_ret =mkt_excess,
...
positive_index = short_term_data_event %>% 
  select(date,isin,#MKT_CAP,
         "0",contains("-"),contains("+"),pos_event) %>% 
  select(-c(contains("pos_event_"),contains("neg_event_"))) %>%
  filter(pos_event == 1) %>%
  select(-pos_event) %>%
  pivot_longer(!c(date,isin,#MKT_CAP
...
negative_index_10_1sd %>% select(type, "AAR_1sd" = AAR, "CAAR_1SD" = CAAR) %>%
  left_join(negative_index_10_2sd %>% select(type, "AAR_2sd" = AAR, "CAAR_sd2" = CAAR), by ="type") %>%
  left_join(negative_index_10_3sd %>% select(type, "AAR_3sd" = AAR, "CAAR_sd3" = CAAR), by ="type") %>%
...
negative_index_10_v %>% select(type, "AAR_v" = AAR, "CAAR_v" = CAAR) %>%
