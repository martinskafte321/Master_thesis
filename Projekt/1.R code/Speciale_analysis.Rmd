...
# Data connection to get weekly market returns for STOXX 600 Europe
...
stoxx600_index_daily <- read.csv("C:/Users/Marti/OneDrive - University of Copenhagen/KU/Speciale/Data behandling/STOXX600_index.csv", sep = ",") %>%
...
data_daily <- read.csv("C:/Users/Marti/OneDrive - University of Copenhagen/KU/Speciale/Data behandling/data_daily.csv", sep = ",") %>%
  ...
  mutate(date = as.Date(date)) %>%
  left_join(stoxx600_index_daily %>% select(date, mkt_excess), by = "date")

short_term_data_nested <- data_daily %>% ungroup() %>%
  select(date,isin,W_RETURN,mkt_excess) %>%
  nest(data = -c(isin)) %>%
  mutate(beta = future_map(
    data, ~ roll_capm_estimation(.,window = 120, min_obs = 50)
  )) %>%
  unnest(c(beta)) %>%
